...
WindownsManager::WindownsManager(MainWindow* par)
{
	...
	connect(multiwindowstabwidget, &QTabWidget::tabCloseRequested, this, &WindownsManager::CloseWindown);//关闭窗口
...
int WindownsManager::CreateNewWindown(string WindownName)
{
	...
	WindownName = fileName.toStdString();
	...
	parent->DisplayCoreManege = new DisplayCore(windowns_name[WindownName]->myOccView);
	...
	string partname = filePath.toStdString();
	parent->DisplayCoreManeger[WindownName]->Read_step_file(partname, WindownName);

	//multiwindowstabwidget
...
int WindownsManager::CloseWindown()
{
	...
	if (tabWidget) 
	{
		...
		QString WindownName = multiwindowstabwidget->tabText(index);
		windowns_name.erase(WindownName.toStdString());//删除对应窗口
		parent->DisplayCoreManeger.erase(WindownName.toStdString());//删除对应窗口的DisplayCoreManeger
		windowns_id_list.pop_back();//删除ID
...
{	
	...
	if (multiwindowstabwidget->count()!=0)
	{
